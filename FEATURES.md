...
| Prod-ready windowed history pruning (v0.13.80) | `src/session/history.rs` | Two improvements: (1) **Drop-summary injection** — when turns fall off the 8-turn sliding window, a synthetic summary message pair is prepended to the windowed slice so the LLM retains key decisions from early turns (e.g. "Turn 1: how do I add UUID support → I added UUID…"). Summary shows up to 5 most-recent dropped turns with user preview (200 chars), assistant reply preview (250 chars), and tool names used. No LLM call — built from raw message text. (2) **Tool result pruning preview** — old `[pruned — N chars]` stub now includes the first 150 chars of the original content so the LLM has a hint about what it saw. 4 new unit tests cover: preview in pruned results, no summary injected within window, summary injected when window slides, correct message count after injection. |
