...
| search_code Windows fix + smart tool previews (v0.13.27) | `src/tools/search/`, `src/session/mod.rs:smart_tool_preview` | (1) Pure-Rust fallback search when rg and grep are both unavailable: walks directory tree via `std::fs`, supports regex (via `regex` crate) and fixed-string, restricts to known text extensions by default (avoids reading binary/.db/.lock files), context lines use `-` separator and match lines use `:` separator (matching rg/grep format), deduplicates context between adjacent matches. (2) `smart_tool_preview()` replaces generic 10-line truncation with tool-specific one-liners. (3) `search.rs` refactored to `search/mod.rs` + `search/search_impl.rs` directory module. |
