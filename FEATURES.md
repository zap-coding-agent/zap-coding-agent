...
| Rotating thinking words | `src/tui/render.rs:THINKING_WORDS` | 192-word rotation across 12 semantic groups (cognitive, creative, analytical, problem-solving, code, organizing, retrieval, exploratory, verification, clarifying, meta); tool-running shows context-aware verb + file label in chat, status bar, and sidebar; word changes every ~1.3s |
| Vibe/Task mode in TUI | `src/tui/mod.rs:run_tui` | Mode picker now runs before alternate screen in TUI mode (same as REPL); Task mode triggers full task-planning flow (clarifying Qs → LLM plan → tasks.md) and primes the first TUI turn; welcome message shows current mode |
