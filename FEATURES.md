...
| `compute_reference_counts()` | `src/code_index.rs` | word-frequency HashMap pass O(source_size); updates `ref_count` column for all symbols in one batch transaction after each index run. High-coupling query excludes trait methods (GROUP BY name HAVING COUNT=1) and short names (LENGTH>=5) to avoid false positives like `name` showing 962 raw word-frequency hits. |
