...
| Auto-init on startup | `src/tui/mod.rs`, `src/project.rs:understanding_needs_analysis` | New projects with any files skip the language wizard and auto-detect + auto-init silently. Returning projects where `understanding.md` has only a placeholder also trigger a background analysis turn. Falls back to mode picker if init produces no LLM prompt. |
