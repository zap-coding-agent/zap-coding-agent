...
| /init analysis bounded to 12 tool calls | `src/session/commands.rs:cmd_init_direct` | Init LLM prompt now has explicit tool budget (max 12 calls) and forbidden patterns (no recursive listing, no glob **\/*, no reading 8+ files, no enumerating all files with sizes). Prevents hanging on large projects where LLM was trying to enumerate everything. |
| Silent language detection on first launch | `src/tui/mod.rs` | New projects: auto-detect language, silently save `project.json`, then proceed — no wizard, no indexing, no LLM call. Indexing and analysis only happen when user explicitly runs `/init`. Truly empty dirs get an empty `project.json` so detection is skipped next session. Unindexed projects see a one-time tip: "run /init to index — code stays local". |
