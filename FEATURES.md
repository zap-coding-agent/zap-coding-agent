...
| Exit hang fix — 5s summary timeout (v0.13.45) | `src/session/commands/code.rs` | `save_context_with_summary` is called after the TUI exits (terminal already restored); with a 20s LLM timeout the process appeared hung after Ctrl+Q/C. Reduced to 5s — cloud APIs respond in 1-3s for this small prompt; on timeout the function falls back gracefully to saving existing context without a new summary |
