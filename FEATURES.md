...
| search_code Windows fix + smart tool previews (v0.13.27) | `src/tools/search.rs`, `src/session/mod.rs:smart_tool_preview` | (1) Pure-Rust fallback search when rg and grep are both unavailable (Windows default): walks directory tree via `std::fs`, supports regex (via `regex` crate) and fixed-string, respects file_type filter, context_lines, max_results — shows `(native, N matches in M files)` in header. (2) `smart_tool_preview()` replaces generic 10-line truncation: `find_definition` shows `🗄 N hits [index]`, `search_code` shows `🔍 N matches (rg/grep/native)`, `code_map` shows `🗄 M files · N symbols [index]` — makes code-index usage visible in the TUI at a glance. |
