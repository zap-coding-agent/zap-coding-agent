...
| Auto-init on startup | `src/tui/mod.rs`, `src/project.rs:understanding_needs_analysis` | New projects with any files skip the language wizard and auto-detect + auto-init silently. Returning projects are left alone — no auto-init triggered without user request. Mode picker popup removed from startup; `ModePickerState` kept for future `/task` slash command. |
